	...
	for {
		...
		case b := <-a.notfy:
			...
			// Starting with elastic.v5, you must pass a context to execute each service
      		...
      		client, err := elastic.NewClient(elastic.SetSniff(false), elastic.SetURL("http://172.16.3.9:9200"))
      		if err != nil {
        		// Handle error
        		panic(err)
      		}
			...
			for i, r := range b.Block.Data.Data {
				...
				if tx != nil {
					...
					if txsFltr.IsInvalid(i) {
						...
						fmt.Printf("Received channel_header from channel %s: \n\t[%v]\n", chdr.ChannelId, tx)
						fmt.Println("------------------------")
						//fmt.Printf("Received signature_header from channel %s: \n\t[%v]\n", chdr.ChannelId, tx.Header.SignatureHeader)
						//fmt.Println("------------------------")
						//fmt.Printf("Received data from channel %s: \n\t[%v]\n", chdr.ChannelId, tx.Data)

						events := Event{Channel_header: string(tx.Header.ChannelHeader) , Signature_header: string(tx.Header.SignatureHeader), Data: string(tx.Data), Created: createtime}
						//events := Event{Channel_header: hex.EncodeToString(tx.Header.ChannelHeader[:]) , Signature_header: hex.EncodeToString(tx.Header.SignatureHeader[:]), Data: hex.EncodeToString(tx.Data[:]), Created: createtime}
						//events := Event{Channel_header: fmt.Sprintf("%x", tx.Header.ChannelHeader) , Signature_header: fmt.Sprintf("%x", tx.Header.SignatureHeader), Data: fmt.Sprintf("%x", tx.Data), Created: createtime}
